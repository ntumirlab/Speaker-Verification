...
class Helper:
    ...
    def plot_tsne(data, labels, feat_name, output_path):
        ...
        plt.scatter(X_tsne[:, 0], X_tsne[:, 1], c=labels, cmap=plt.cm.get_cmap("jet", labels[-1]+1))
        plt.savefig(output_path)
        plt.cla()
        plt.clf()

    @staticmethod
    def plot_probabilty_density(p_scores, n_scores, output_path, threshold, fa, fr, Cprim, Cthr):
        sns.set()
